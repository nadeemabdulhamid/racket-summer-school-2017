...
(define (fold-funcs apl bf-funcs)   ; apl = array + pointer list (pair)
...
(define-macro (define-op NAME OP)
  #'(define (NAME arr ptr)
      (list
       (set-current-byte arr ptr (OP (current-byte arr ptr)))
       ptr)))

(define-op plus add1)
(define-op minus sub1)
